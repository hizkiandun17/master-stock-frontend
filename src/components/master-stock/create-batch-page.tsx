...
import type { BatchLine, StockLocationKey } from "@/lib/types";
...
export function CreateBatchPage() {
  ...
  const { products, categories, createBatch } = useMasterStock();
  const [name, setName] = useState("");
  const [source, setSource] = useState<StockLocationKey>("indira");
  const [items, setItems] = useState<BatchLine[]>([]);
  ...
  function addProduct(productId: string) {
    setItems((current) => [
      ...current,
      {
        id: `line-${productId}-${current.length + 1}`,
        productId,
        plannedQty: 0,
        receivedQty: 0,
        checked: false,
  ...
  function updateItem(productId: string, patch: Partial<BatchLine>) {
    setItems((current) =>
      current.map((item) =>
        item.productId === productId
          ? {
              ...
              ...patch,
              plannedQty:
                patch.plannedQty === undefined ? item.plannedQty : Math.max(0, patch.plannedQty),
              receivedQty:
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm text-muted-foreground md:text-base">
                Create a receiving checklist for incoming production stock.
              </p>
            </div>
          </div>
